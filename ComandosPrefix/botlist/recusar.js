...
if(!message.member.roles.cache.get(role)) return message.reply("Você não pode usar esse comando!")
    ...
    if(!bt == null) return message.reply(`Esse bot não existe!!!`)
...
await db.delete(`prefix_${message.guild.id}_${bot.id}`)
await db.delete(`desc_${message.guild.id}_${bot.id}`)
    message.channel.send({content: "Bot reprovado!!!"})
